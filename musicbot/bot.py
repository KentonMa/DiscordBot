...
class MusicBot(discord.Client):
    ...
    async def cmd_counters(self, message):
        """
        Usage:
            {command_prefix}counters champion_name role

        Get counters to a champion
        """
        #TODO usage not showing
        split = message.content.split(" ")
        if (len(split) < 3):
            return Response("Usage: !counters <champion_name> <role=[MID,TOP,SUPPORT,JUNGLE,ADC]>", delete_after=20)
        else:
            mgg = MatchUpGGCrawler()
            counters = mgg.get_counter_champs(split[1], split[2])
